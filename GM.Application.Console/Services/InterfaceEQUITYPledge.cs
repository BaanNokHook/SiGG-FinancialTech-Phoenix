...
namespace GM.Application.Console.Services
{
    ...
    public class InterfaceEQUITYPledge : WebAPI
    {

        private DateTime systemDate;   
        private IConsoleService _service;  
        private static Console_Entity ConsolEnt = New Console_Entity();   
        private static Mail_AdminEntity MailAdminEnt = new MailAdminEnt();  

        public InterfaceEQUITYPledge(IHttpClientFactory httpClientFactory, IConsoleService consoleService) : base(httpClientFactory)
        {
        ...
        public bool Run()  
        {
            ...
            try
            {
                ...
                var ResultRpconfig = StaticAPI.RpConfig.GetRpConfig("RP_EQUITY_INTERFACE_PLEDGE", string.Empty);
                ...
                if (!SetConfigInterfaceFITSBondPledge(ref StrMsg, ref equityPledgeModel, rpConfigModel))
                {
                    throw new Exception("SetConfigInterfaceFITSBondPledge() => " + StrMsg);
                }
                ...
                // Step 4 : Interface EQUITY BondPledge
                _service.WriteLogs("");
                _service.WriteLogs("Run InterfaceEQUITYPledge");
                var resultEquityPledge = ExternalInterfaceAPI.InterfaceEquityPledge.InterfaceEquityPledge(equityPledgeModel);
                if (!resultEquityPledge.Success)
                {
                    throw new Exception("InterfaceEquityPledge() => [" + resultEquityPledge.RefCode.ToString() + "] " + resultEquityPledge.Message);
                }

                _service.WriteLogs("ReturnCode = [" + resultEquityPledge.RefCode + "] " + resultEquityPledge.Message);
                _service.WriteLogs("InterfaceEquityPledge Success.");
            }
            catch (Exception Ex)
            {
        ...
        private bool SetConfigInterfaceFITSBondPledge(ref string StrMsg, ref InterfaceEquityPledgeModel equityPledgeModel, List<RpConfigModel> List_RpConfigModel)
        {
            try
            {
                ...
                {
                    equityPledgeModel.ServiceTimeOut = Convert.ToInt32(List_RpConfigModel.FirstOrDefault(a => a.item_code == "SERVICE_TIMEOUT")?.item_value);
                }

                equityPledgeModel.AsOfDate = systemDate.Date;
                ...
                _service.WriteLogs("ServiceUrl       = " + equityPledgeModel.ServiceUrl);
                _service.WriteLogs("ServiceTimeOut   = " + equityPledgeModel.ServiceTimeOut);
                ...
                _service.WriteLogs("AsOfDate         = " + equityPledgeModel.AsOfDate.ToString("yyyyMMdd"));
