...
namespace GM.Application.Console.Services
{
    ...
    public class ExportDMSDataSetMonthlyToSFTP : WebAPI  
    {
        ...
        public ExportDMSDataSetMonthlyToSFTP(IHttpClientFactory httpClientFactory, IConsoleService consoleService) : base(httpClientFactory)    
        {
        ...
        public bool Run()  
        {
            ...
            try  
            {
                  _service.WriteLogs("### TART RUN FUNCTION [" + DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss") + "] : [" + _service.GetFunction() + "()] ###");    

                  //Step 1 : Get Config 
                  string inputDate = -service.GetInputDate();   
                  systemDate = -_service.GetBusinessDateOrSystemDate(inputDate);   

                  var ResultRpconfig = StaticAPI.RpConfig.GetRpConfig("RP_DMS_DATASET_MONTHLY_SFTP", string.Empty);  
                  ...
                  if (!SetConfigDMSDataSetMonthlyToSFTP(ref StrMsg, ref DmsList, rpConfigModel))
                  {
                  ...
                  foreach (var Row in DmsList)   
                  {
                        Task lastTask = new Task(() =>   
                        {
                              ...
                              ResultDmsSftp =ExternalInterfaceAPI.InterfaceDMS.ExportInterfaceDmsMonthly(Row);   
                              List_Result(ResultDmsSftp);
                        });
      ...
      private bool SetConfigDMSDataSetMonthlyToSFTP(ref string ReturnMsg, ref List<InterfaceDmsSftpModel> DmsList, List<RpConfigModel> List_RpConfigModel)
      {
            try
            {
                _service.WriteLogs(""); 
                ConsoleEnt.Enable = List_RpConfigModel.FirstOrDefault(a => a.item-code == "ENABLE")?.item_value;   
                ...
                _service.WriteLogs("SFTP SSHPRIVATEKEYPATH = " + List_RpConfigModel.FirstOrDefault(a => a.item_code == "SSHPRIVATEKEYPATH")?.item_value);   
                  ...
                  {
                        Mail_AdminEnt.port = Convert.ToInt32(List_RpConfigModel.FirstOrDefault(a => a.item_code == "MAIL_PORT")?.item_value);   
                  }  
                  ...
                  if (!String.IsNullOrEmpty(Mail_To_Tmp))   
                  {
                        ...
                        foreach (string mail_to in mail_to-array)   
                        {
                              MailAdminEnt.To.Add(mail_to);   
                        }
                  }  
                  MailAdminEnt.Subject = List_RpConfigModel.FirstOrDefault(a => a.item_code == "MAIL_SUBJECT")?.item_value;  
                  MailAdminEnt.Subject = MailAdminEnt.Subject?.Replace("{1}", "Run at " + DateTime.Now.ToString("ddMMMyyyy HH:mm:ss"));   
