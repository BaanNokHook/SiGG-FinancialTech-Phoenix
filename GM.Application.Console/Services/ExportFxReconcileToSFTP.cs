...
namespace GM.Application.Console.Services
{
   ...
   public class ExportFxReconcileToSFTP : WebAPI  
   {
      ...
      public ExportFxReconcileToSFTP(IHttpClientFactory httpClientFactory, IConsoleService consoleService) : base(httpClientFactory)
      {
      ...
      public bool Run()  
      {
            ...
            try
            {
                  ...
                  var ResultRpconfig = StaticAPI.RpConfig.GetRpConfig("RP_FX_RECONCILE_SFTP", string.Empty);   
                  ...
                  if (!SetConfigFxReconcileToSFTP(ref StrMsg, ref FxReconcileSftpModel, rpConfigModel))   
                  {
                  ...
                  _service.WriteLogs("");  
                  _service.WriteLogs("Run ExportInterfaceReconcile");   

                  var ResultFxReconcileSftp = ExternalInterfaceAPI.InterfaceFx.ExportInterfaceFxReconcile(FxReconcileSftpModel);   
                  if (!ResultFxReconcileSftp.Success)
                  {
                        throw new Exception("ExportTransLimitCr() => [" + ResultFxReconcileSftp.RefCode.ToString() + "] " + ResultFxReconcileSftp.Message);   
                  }  

                  InterfaceFxReconcileSftpModel resultModel = ResultFxReconcileSftp.Data[0];   
        ...
        private bool SetConfigFxReconcileToSFTP(ref string ReturnMsg, ref InterfaceFxReconcileSftpModel FxReconcileSftpModel, List<RpConfigModel> List_RpConfigModel)
        {
            try
            {
                _service.WriteLogs("");
                ...
                _service.WriteLogs("FileTransaction = " + List_RpConfigModel.FirstOrDefault(a => a.item_code == "FILE_NAME_TRANSACTION")?.item_value);
                _service.WriteLogs("FilePosition = " + List_RpConfigModel.FirstOrDefault(a => a.item_code == "FILE_NAME_POSITION")?.item_value);
                _service.WriteLogs("FilePostingEvent = " + List_RpConfigModel.FirstOrDefault(a => a.item_code == "FILE_NAME_POSTINGEVENT")?.item_value);

                FxReconcileSftpModel.AsofDate = systemDate.Date;
